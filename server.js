...
const { initDb, fetchTopScores, insertScore, incrementStat, getStats, insertFeedback } = require('./db');
...
function sanitizeFeedbackMessage(raw) {
  if (typeof raw !== 'string') return null;
  const trimmed = raw.trim().slice(0, FEEDBACK_MAX_LEN);
  return trimmed.length ? trimmed : null;
}
...
app.post('/api/feedback', async (req, res) => {
  const message = sanitizeFeedbackMessage(req.body && req.body.message);
  if (!message) return res.status(400).json({ error: 'invalid message' });

  let contact = null;
  if (typeof (req.body && req.body.contact) === 'string') {
    contact = req.body.contact.trim().slice(0, CONTACT_MAX_LEN) || null;
  }

  try {
    await insertFeedback(message, contact);
    res.json({ ok: true });
  } catch (err) {
