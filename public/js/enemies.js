...
import { BOUND_X, BOUND_Y_MIN, BOUND_Y_MAX, ENEMY_HIT_RADIUS, ENEMY_MISS_RADIUS, GROWTH_ENEMY_HP_PER_TIER, enemyGrowthFactor } from './constants.js';
...
export function spawnEnemy(waveConfig) {
  ...
  scene.add(sprite);
  const hp = state.growthMode ? 1 + GROWTH_ENEMY_HP_PER_TIER * enemyGrowthFactor(state.level) : 1;
  enemies.push({
    mesh: sprite,
